...
class LoadImagesAndLabels(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # zxj
        ndarray_list = np.array(self.segments[index].copy())
        # if torch.size(ndarray_list)==0:
        #     a=1
        additional_tensor = ndarray_list[:, 2:, :]
        additional_tensor_ = additional_tensor.reshape(ndarray_list.shape[0], -1)
        additional_tensor_reshaped_torch = torch.tensor(additional_tensor_, dtype=torch.float32)
        labels_out = torch.cat((labels_out, additional_tensor_reshaped_torch), dim=1)
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                ...
                if any(len(x) > 6 for x in lb):  # is segment
                    ...
                    # lb = np.concatenate((classes.reshape(-1, 1), segments2boxes(segments)), 1)  # (cls, xywh) #zxj
                lb = np.array([np.array(x[:5], dtype=np.float32) for x in lb])
