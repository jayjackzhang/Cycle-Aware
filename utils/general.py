...
# with open('../datasets/train_mul/labels/names.txt', 'r') as file:
#     # 初始化一个空字典
#     data_dict = {}
#     # 逐行读取文件内容
#     for line in file:
...
# with open('../datasets/names_all.txt', 'r') as file:
#     # 初始化一个空字典
#     data_dict = {}
#     # 逐行读取文件内容
#     for line in file:
...
#         tensor = torch.tensor([int(char) for char in value])
#         # 将键值对添加到字典中
#         data_dict[int(''.join(map(str, tensor.tolist())), 2)] = int(key)
...
def non_max_suppression(
    ...
    cosface_weight = None,bn_embed = None
):
    """
    ...
    x_cat_list = []
    for xi in raw_out_:
        bs, na, ny, nx, no = xi.shape
        x_cat_list.append(xi.view(bs, na * ny * nx, no))
    x_cat = torch.cat(x_cat_list, 1)  # [bs, total_boxes, no]
    # Settings
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            i, j = (x[:, 5:mi] > 0.5).nonzero(as_tuple=False).T
            x = torch.cat((box[i], x[i, 5 + j, None], j[:, None].float(), mask[i]), 1)
        else:  # best class only

            cal_type = 1
            if cal_type == 1 or  cal_type == 2:
                # predictions = (torch.sigmoid(x[:, 5:mi]) > conf_thres).int()
                predictions = ((x[:, 5:20]) > 0.5).int()
                integer_tensor = torch.zeros(predictions.shape[0], dtype=torch.int32)
                for i in range(predictions.shape[0]):
                    integer_tensor[i] = int(''.join(map(str, predictions[i].tolist())), 2)
                integer_tensor = integer_tensor.view(predictions.shape[0], 1)
                integer_tensor=integer_tensor.to('cuda')
                # # 将二进制张量转换为整数张量
                # integer_tensor_ = torch.zeros(predictions.shape[0], dtype=torch.int32)
                # integer_tensor_ = torch.pack(predictions.tolist(), dim=1).view(-1, 1)
                acc=x[:, 5:20]
                mask = (predictions==0)
                acc[mask]=1 - acc[mask]
                conf = acc.mean(dim=1).unsqueeze(-1)
                conf2 = x[:, 4].float()
                # conf = (predictions * x[:, 5:20]).mean(dim=1)+((1-predictions) * (1-x[:, 5:20])).mean(dim=1)
                # conf, j = x[:, 5:20].max(1, keepdim=True)
                x = torch.cat((box, conf, integer_tensor.float(), conf2.unsqueeze(-1)), 1)

            if cal_type == 3:
                # 提取嵌入 (B, 15)
                pred_embed = x[:, 5:].float()
                probs = torch.softmax(pred_embed, dim=1)
                pred_class = torch.argmax(probs, dim=1).unsqueeze(1)
                # 组装输出 [xyxy, conf, cls, obj_conf]
                obj_conf = x[:, 4:5]  # 原始 objectness
                x = torch.cat((box, obj_conf, pred_class.float(), obj_conf), 1)

            # if cal_type == 5 or  cal_type == 6:
            #     raw_embed = raw_out[:,5:].float()
            #     # normed_embed = bn_embed(raw_embed)
            #     normed_embed = F.normalize(raw_embed, p=2, dim=1)
            #     w = F.normalize(cosface_weight, p=2, dim=1)
            #     cosine_scores = normed_embed @ w.T
            #     probs = torch.softmax(cosine_scores * 30, dim=1)
            #     cls_conf, pred_class = probs.max(dim=1)
            #     obj_conf = x[:, 4]
            #     conf = obj_conf * cls_conf
            #     x = torch.cat((box, conf.unsqueeze(-1), pred_class.unsqueeze(-1), obj_conf.unsqueeze(-1)), 1)
